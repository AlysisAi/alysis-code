...
async function main(): Promise<void> {
  ...
  if (!workspacePath || !vscodeExecutablePath) {
    throw new Error("ALYSIS_LIVE_QA_WORKSPACE and VSCODE_TEST_EXECUTABLE_PATH are required.");
  }
  let qaSecret = takeEnvironmentSecret(LIVE_API_KEY_ENVIRONMENT);
  restrictLiveQaEnvironment(process.env, qaSecret);
  const userDataDir = mkdtempSync(join(tmpdir(), "alysis-live-soak-user-data-"));
  const extensionsDir = mkdtempSync(join(tmpdir(), "alysis-live-soak-extensions-"));
  ...
  try {
    ...
    extensionTestsEnv.ALYSIS_LIVE_SOAK = "1";
...
main().catch((error) => {
  console.error(JSON.stringify({ evidence: "live_soak_failure", error: safeErrorEvidence(error) }));
  process.exit(1);
});
